 ...
 *
 * If the blob store is unavailable the request is ALLOWED. Somebody whose
 * dryer vent is a fire risk should not be turned away because a counter is
 * down, and the daily cap is the thing standing between us and a real bill
 * anyway.
 */
...
export async function check(request: Request): Promise<Verdict> {
  ...
  try {
    ...
    /* Fail open. See the note at the top: a counter being down is not a
       reason to refuse someone whose dryer will not dry. */
    return { ok: true };
  }
}
